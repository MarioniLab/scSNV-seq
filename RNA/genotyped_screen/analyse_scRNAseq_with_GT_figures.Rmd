...
```
This notebook was used to analyse the RNA modality of the genotyped data set after QC and gRNA calling. 
...
gRNAs <-  matrix(0,nrow=nrow(knns$index),ncol=ncol(knns$index))
  for (j in 1:nrow(knns$index))
  {
    xx <- match(k.mapped[j,], colnames(sce_bigBE))
    gRNAs[j,] <- sce_bigBE$gRNA[xx]
  }
  
  gRNA.mapped <- rep("",nrow(gRNAs))
  for (j in 1:nrow(gRNAs)){
    gRNA.mapped[j] <- getmode(gRNAs[j,],1:ncol(gRNAs))
  }

  
reducedDims(sce_knownGT)$UMAP <- reducedDims(sce_bigBE)$UMAP[sce_knownGT$nearest_bigBE_cell,]
sce_knownGT$cluster <- cluster.mapped
sce_knownGT$mapped_gRNA <- gRNA.mapped
...
We plot the distribution of the different possible consequences across the two clusters. 
...
Repeating the above plot with predicted consequence instead of consequence

```{r predicted_cluster_consequence_filtered}

print(ggplot(mapping=aes(x=sce_temp$cluster,fill=sce_temp$predicted_consequence)) +geom_bar(aes( y=..count../tapply(..count.., ..x.. ,sum)[..x..])) +
     ylab("") +labs(fill="") + xlab("cluster") + theme_bw(base_size=15)+ggtitle("Consequence (VEP, based on prediction)")+
  scale_fill_manual(values=colours_consequence) + theme(legend.position = "bottom") + guides(fill = guide_legend(nrow = 4)))

```
We compare the distribution of actual and predicted consequences across clusters.

```{r}
levels <- unique(c(sce_temp$predicted_consequence,sce_temp$consequence))
...
df_row_cor$predicted_consequence[is.na(df_row_cor$predicted_consequence)] <- "WT"
df_row_cor$predicted_consequence[df_row_cor$predicted_consequence=="not predicted"] <- "WT"# cells with a gRNA not targeting JAK1
my_colour = list(
    consequence = colours_consequence,
    predicted_consequence=c(colours_consequence,"not predicted"="white"),
